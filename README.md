...
2. Extract the files into the folder where the game's exe (`POP.exe`) lives.
   ...
   If you still want to use my SDL2 solution, open `gog.toml` (the one in the game folder) via Notepad and change `#'dinput8.dll' = 'dinput8_wrap.dll'` to `'dinput8.dll' = 'dinput8.dll'`.
...
them in-game will conflict with that.

Unlike its sequel, this game doesn't let you navigate menus with a
controller, so I've added a special controller mode that lets you move
the mouse:

| Button | Action |
|---|---|
| **Start** | Enter/exit the pause menu (acts as the Escape key) |
| **Select** | Toggles mouse movement via the right stick. In this mode the Prince can't move or perform actions - it's built exclusively for working the in-game menu. Square/X is used as the left mouse button. Turn it off again once you're back in gameplay to return control to the Prince. |

See `[MenuMode]` in the ini reference below to change the buttons or
tune it, or turn the whole thing off with `Enable=0`.
...
The game can't navigate menus with a controller at all (unlike its sequel).
This block adds two things, both switched off together by `Enable=0`:
`EscapeButton` always acts as Escape/pause in every mode (via scan-code
`SendInput`, since this DirectInput-era game doesn't respond to a plain
virtual-key Escape), and `ToggleButton` flips a virtual mouse on/off where
the right stick drives the real Windows cursor and `ClickButton` fires
left-clicks - completely separate from DirectInput, so it works the same
as any other application moving your mouse/keyboard.

| Key | Default | What it does |
|---|---|---|
| `Enable` | `1` | Turn the whole block on/off - both the Escape hotkey and the mouse toggle. |
| `EscapeButton` | `start` | Always sends Escape/pause, in every mode. Suppressed from normal button forwarding to the game while this is on. |
| `ToggleButton` | `back` | Toggles the virtual mouse on/off. While active, the Prince can't move or act - this mode is for working the in-game menu only. Valid names for any of these three: `a`, `b`, `x`, `y`, `back`, `guide`, `start`, `leftstick`, `rightstick`, `leftshoulder`, `rightshoulder`, `dpup`, `dpdown`, `dpleft`, `dpright`. |
| `ClickButton` | `x` | Fires a left mouse click while the mouse toggle is active. Ignored otherwise. Defaults to X/Square to mirror keyboard+mouse controls, where left-click is also the sword attack. |
| `CursorSpeed` | `900` | Pixels/second of real cursor movement at full right-stick deflection. |
| `Deadzone` | `0.15` | Radial deadzone on the right stick while the mouse toggle is active. |
| `InvertY` | `0` | Invert the cursor's vertical movement. |

While the mouse toggle is active, the game receives a neutral (all-zero)
pad state instead of your real input, so `ToggleButton`/`ClickButton`
don't also trigger whatever they're bound to in-game. **Don't rebind
`EscapeButton`'s or `ToggleButton`'s physical buttons (Start/Select by
default) to anything else in the game's own Controls menu** - see the
warning in Quick Start above.

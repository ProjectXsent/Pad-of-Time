...
void Config_Load(const char* iniName)
{
    ...
    g_cfg.enableMenuMode = GetB("MenuMode", "Enable", g_cfg.enableMenuMode, p);
    {
        char buf[32];
        GetPrivateProfileStringA("MenuMode", "EscapeButton", g_cfg.escapeButton.c_str(), buf, sizeof(buf), p);
        g_cfg.escapeButton = buf;
        GetPrivateProfileStringA("MenuMode", "ToggleButton", g_cfg.mouseToggleButton.c_str(), buf, sizeof(buf), p);
        g_cfg.mouseToggleButton = buf;
        GetPrivateProfileStringA("MenuMode", "ClickButton", g_cfg.mouseClickButton.c_str(), buf, sizeof(buf), p);
        g_cfg.mouseClickButton = buf;
    }
    g_cfg.menuCursorSpeed = GetF("MenuMode", "CursorSpeed", g_cfg.menuCursorSpeed, p);
    g_cfg.menuDeadzone    = GetF("MenuMode", "Deadzone",    g_cfg.menuDeadzone,    p);
    g_cfg.menuInvertY     = GetB("MenuMode", "InvertY",     g_cfg.menuInvertY,     p);
